...
static void stabilizationTask(void* parameters)
{
	...
	static uint8_t gyroSubsample = 0; // init the gyro sub sampling counter
	...
	while(1) {
		...
		if (( ++gyroSubsample % settings.GyroSubSampling ) == 0) {
			gyroSubsample = 0; // reset the gyro sub sampling counter
